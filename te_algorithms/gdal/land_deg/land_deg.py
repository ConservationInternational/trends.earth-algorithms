...
def _accumulate_ld_summary_tables(
        tables: List[models.SummaryTableLD]) -> models.SummaryTableLD:
    ...
    for table in tables[1:]:
        out.soc_by_lc_annual_totals = [
            util.accumulate_dicts([a, b]) for a, b in zip(
                out.soc_by_lc_annual_totals, table.soc_by_lc_annual_totals)
        ]
...
def _prepare_land_cover_dfs(params: Dict) -> List[DataFile]:
    ...
    lc_dfs = [
        DataFile(path=util.save_vrt(lc_path,
    ...
    for lc_aux_band, lc_aux_band_index, in zip(
        ...
        lc_dfs.append(
            DataFile(path=util.save_vrt(lc_path, lc_aux_band_index),
                     bands=[JobBand(**lc_aux_band)]))
    lc_dfs.append(
        DataFile(path=util.save_vrt(
...
def _prepare_population_df(params: Dict) -> DataFile:
    ...
    population_df = DataFile(
        path=util.save_vrt(population_path,
...
def _prepare_soil_organic_carbon_dfs(params: Dict) -> List[DataFile]:
    ...
    soc_dfs = [
        DataFile(path=util.save_vrt(soc_path,
    ...
    for soc_aux_band, soc_aux_band_index, in zip(
        ...
        soc_dfs.append(
            DataFile(path=util.save_vrt(soc_path, soc_aux_band_index),
...
def _prepare_jrc_lpd_mode_df(params: Dict) -> DataFile:
    return DataFile(path=util.save_vrt(params["layer_lpd_path"],
                                       params["layer_lpd_band_index"]),
                    bands=[JobBand(**params["layer_lpd_band"])])


def _compute_summary_table_from_te_prod(
        aoi, in_dfs, lc_legend_nesting: land_cover.LCLegendNesting,
        ...
        output_job_path: Path, period_name, periods,
        compute_bbs_from) -> Tuple[models.SummaryTableLD, Path]:
...
def summarise_land_degradation(ldn_job: Job, aoi: AOI,
                               job_output_path: Path) -> Job:
    ...
    for period_name, period_params in ldn_job.params.items():
        ...
        sdg_band = JobBand(name=config.SDG_BAND_NAME,
                           no_data_value=config.NODATA_VALUE,
                           metadata={
                               'year_initial':
                               period_params['period']['year_initial'],
                               'year_final':
    ...
    save_reporting_json(summary_json_output_path, summary_tables,
                        progress_summary_table, ldn_job.params,
                        ldn_job.task_name, aoi, summary_table_stable_kwargs)
...
def _process_block_summary(params: models.DegradationSummaryParams, in_array, mask,
                           xoff: int, yoff: int,
                           cell_areas_raw) -> Tuple[models.SummaryTableLD, Dict]:

    lc_band_years = params.in_df.metadata_for_name(config.LC_BAND_NAME, 'year')
    lc_bands = [(band, year) for band, year in zip(
        params.in_df.indices_for_name(config.LC_BAND_NAME), lc_band_years)]
    soc_bands = [(band, year) for band, year in zip(
    ...
    a_lc_trans_lc_deg = calc_lc_trans(in_array[lc_deg_initial_cover_row, :, :],
    ...
    elif (prod_deg_band_period['year_initial'] in lc_band_years
    ...
    elif (soc_deg_band_period['year_initial'] in lc_band_years
    ...
    if a_lc_trans_prod_deg is not None:
        lc_trans_prod_bizonal = bizonal_total(a_lc_trans_prod_deg, deg_prod5,
                                              cell_areas, mask)
    else:
    ...
    return (models.SummaryTableLD(soc_by_lc_annual_totals, lc_annual_totals,
                           lc_trans_zonal_areas, lc_trans_zonal_areas_periods,
                           lc_trans_prod_bizonal, lc_trans_zonal_soc_initial,
                           lc_trans_zonal_soc_final,
                           sdg_zonal_population_total, sdg_summary,
                           prod_summary, soc_summary, lc_summary),
            write_arrays)
...
def _calculate_summary_table(
    ...
    indic_vrt = tempfile.NamedTemporaryFile(suffix='_ld_summary_inputs.vrt',
                                            delete=False).name
    logger.info(f'Saving indicator VRT to: {indic_vrt}')
    gdal.BuildVRT(indic_vrt, [item.path for item in in_dfs],
    ...
    if reproject_worker_function:
        reproject_result = reproject_worker_function(indic_vrt,
    ...
    if reproject_result:
        ...
        # the VRT
        mask_tif = tempfile.NamedTemporaryFile(suffix='_ld_summary_mask.tif',
                                               delete=False).name
        logger.info(f'Saving mask to {mask_tif}')
        geojson = util.wkt_geom_to_geojson_file_string(wkt_aoi)

        if mask_worker_function:
            mask_result = mask_worker_function(mask_tif, geojson,
                                               str(output_layers_path),
                                               **mask_worker_params)

        else:
            mask_worker = workers.Mask(mask_tif, geojson,
                                       str(output_layers_path))
...
def _compute_ld_summary_table(
        ...
        lc_legend_nesting: land_cover.LCLegendNesting,
        lc_trans_matrix: land_cover.LCTransitionDefinitionDeg,
        period_name: str, periods: dict) -> Tuple[models.SummaryTableLD, Path, Path]:
    ...
    for index, (wkt_aoi, pixel_aligned_bbox) in enumerate(zip(wkt_aois, bbs),
                                                          start=1):
