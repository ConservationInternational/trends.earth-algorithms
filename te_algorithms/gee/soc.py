...
def soc(
    ...
    for k in range(year_final - soc_t0_year):
        ...
        if (k == 0):
            ...
            tr_time = tr_time.where(lc_t0.eq(lc_t1), tr_time.add(
                ee.Image(1)
        ...
        if (k == 0):
            soc_chg = (
                soc_t0.subtract(
                    (
                        soc_t0.multiply(lc_tr_fl).multiply(lc_tr_fm).
                        multiply(lc_tr_fo)
                    )
                )
            ).divide(20)
                ...
                soc_chg.where(
                    lc_t0.neq(lc_t1), (
                        stack_soc.select(k).subtract(
                            stack_soc.select(k).multiply(lc_tr_fl).
                            multiply(lc_tr_fm).multiply(lc_tr_fo)
                        )
                    ).divide(20)
