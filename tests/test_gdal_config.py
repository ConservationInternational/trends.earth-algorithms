...
class TestProductivityClassKey:
    ...
    def test_productivity_class_ordering(self):
        ...
        assert PRODUCTIVITY_CLASS_KEY["Declining"] < PRODUCTIVITY_CLASS_KEY["Moderate decline"]
        assert PRODUCTIVITY_CLASS_KEY["Moderate decline"] < PRODUCTIVITY_CLASS_KEY["Stressed"]
...
class TestConfigurationIntegrity:
    ...
    def test_band_names_contain_expected_keywords(self):
        ...
        prod_bands = [PROD_STATUS_BAND_NAME, TRAJ_BAND_NAME, PERF_BAND_NAME, STATE_BAND_NAME]
        ...
        for band in soc_bands:
            assert ("soil" in band.lower() and "carbon" in band.lower()) or "SOC" in band
    ...
    def test_consistent_naming_patterns(self):
        ...
        status_bands = [SDG_STATUS_BAND_NAME, PROD_STATUS_BAND_NAME, 
                       LC_STATUS_BAND_NAME, SOC_STATUS_BAND_NAME]
        for band in status_bands:
            assert "status" in band
            
        # Degradation bands should contain "degradation"
        deg_bands = [PROD_DEG_COMPARISON_BAND_NAME, LC_DEG_BAND_NAME, 
                    LC_DEG_COMPARISON_BAND_NAME, SOC_DEG_BAND_NAME]
    ...
    def test_configuration_completeness(self):
        ...
        required_components = {
            'productivity': [PROD_STATUS_BAND_NAME, TRAJ_BAND_NAME, PERF_BAND_NAME, STATE_BAND_NAME],
        ...
        for component, bands in required_components.items():
            ...
            for band in bands:
                assert isinstance(band, str), f"Band name for {component} is not a string"
...
class TestNumpyCompatibility:
    ...
    def test_productivity_values_work_with_numpy(self):
        ...
        numeric_values = [v for v in PRODUCTIVITY_CLASS_KEY.values() if v != NODATA_VALUE]
