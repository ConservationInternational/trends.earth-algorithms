...
# ruff: noqa: E402
from te_algorithms.gee import productivity, GEEIOError
...
class TestProductivityTrajectory:
    ...
    def test_productivity_trajectory_ndvi_method(self, mock_ndvi_trend, mock_kendall, mock_image, mock_check_validity):
        ...
        mock_lf_trend.select.return_value.abs.return_value.lte.return_value = MagicMock()
        ...
        mock_logger.debug.assert_called_with("Entering productivity_trajectory function.")

    @patch('te_algorithms.gee.util.TEImage._check_validity')
    @patch('te_algorithms.gee.productivity.ee.Image')
    @patch('te_algorithms.gee.productivity.stats.get_kendall_coef')
    @patch('te_algorithms.gee.productivity.p_restrend')
    def test_productivity_trajectory_prestrend_method(self, mock_prestrend, mock_kendall, mock_image, mock_check_validity):
...
class TestProductivityPerformance:
    ...
    def test_productivity_performance_basic(self, mock_geometry, mock_image, mock_check_validity):
...
class TestProductivityFaowocat:
    ...
    def test_productivity_faowocat_missing_year_final(self):
        ...
        with pytest.raises(GEEIOError, match="Must specify 'year_final' for FAO-WOCAT dynamics"):
            productivity.productivity_faowocat(
                year_initial=2001,
                year_final=None,
                prod_asset="test_asset",
