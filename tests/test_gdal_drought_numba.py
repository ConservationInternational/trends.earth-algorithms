...
class TestDroughtClass:
    ...
    def test_drought_class_preserves_nodata(self):
        ...
        spi = np.array([
            [NODATA_VALUE[0], -1000, NODATA_VALUE[0]],
            [-2000, NODATA_VALUE[0], 0]
        ], dtype=np.int16)
...
class TestDroughtClassificationLogic:
    ...
    def test_drought_severity_ordering(self):
        ...
        spi_values = [500, -500, -1250, -1750, -2500]  # Normal, Mild, Moderate, Severe, Extreme
    ...
    def test_scaling_factor_handling(self):
        ...
        expected = np.array([[0, 1, 2, 3]], dtype=np.int16)  # Normal, Mild, Moderate, Severe
        np.testing.assert_array_equal(result, expected)